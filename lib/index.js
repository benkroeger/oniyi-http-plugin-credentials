    ...
    load: (req, origParams, callback) => {
      ...
      user[options.userRelationProp]({ where: { provider: options.providerName } }, (credentialsErr, results) => {
        ...
        if (!Array.isArray(results) || results.length !== 1) {
          ...
          return user[options.userRelationProp].create({
            provider: options.providerName,
            credentials: {},
          }, (createEmptyCredsErr, identity) => {

            if (createEmptyCredsErr) {
              return callback(createEmptyCredsErr);
            }


            return options.initCredentials(user.id, (initCredsErr, initializedCredentials) => {
              if (initCredsErr) {
                return callback(initCredsErr);
              }

              identity.updateAttribute(options.credentialsProp, initializedCredentials, (updateIdentityErr) => {
                if (updateIdentityErr) {
                  return callback(updateIdentityErr);
                }
                return options.makeAuthParams(initializedCredentials, (authParamsErr, authParams) => {
                  if (authParamsErr) {
