    ...
    load: (req, origParams, callback) => {
      ...
      user[options.userRelationProp]({
        ...
        if (!Array.isArray(results) || results.length !== 1) {
          ...
          return options.initCredentials(user.id, (initCredsErr, initCredentialsResults) => {
            if (initCredsErr) {
              return callback(initCredsErr);
            }

            const { credentials, authScheme } = initCredentialsResults;
            user[options.userRelationProp].create({
              ...
              return options.makeAuthParams(credentials, (authParamsErr, authParams) => {
                if (authParamsErr) {
