...
const applyCredentials = (reqParams, { type, payload }) => {
  const { [type]: applyCredentialsImpl } = applyCredentialsByType;

  if (!_.isFunction(applyCredentialsImpl)) {
    throw new Error(`credentials type ${type} is not supported`);
  }

  const reqParamsWithCredentials = applyCredentialsImpl(reqParams, payload);
  return _.defaults(reqParamsWithCredentials, { authType: type });
};

module.exports = { applyCredentials };
